...
void putstring(unsigned char *str)
{
    while(*str){
			...
			// Serial.write()  sends   bytes  to   the  serial   port  while
			// Serial.print() sends  ASCII characters so people  can read it
			// easily.
...
//    nrf905.RX(rx_buf, rx_address );
    ...
    /** send recieved data to PC through serial port */
		...
		// Output actual byte,  ie binary data, values to  the serial port
		// instead of ascii string.  This may  be so that he can trace the
		// values on  SPI output  on the  cro to see  if he's  getting the
		// correct results.
		putstring(rx_buf);
		// Try Serial.print(rx_buff) so see if I can get the same results.
    delay(1);
}
