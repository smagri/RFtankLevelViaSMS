...
// https://github.com/zkemble/nRF905-arduino/blob/master/examples/lowpwr_node_baseStation/lowpwr_node_baseStation.ino
...
// CE (TRX_EN) -> VCC
// TXE (TX_EN) -> GND  Thus, this is the reciver RX.
// PWR -> VCC
// The nRF905 will always be in low-power receive mode, waiting for
// packets from sensor nodes.
...
nRF905 transceiver = nRF905();
 ...
 // NOTE: In interrupt mode this must be volatile
static volatile uint8_t packetStatus;


// define ISR's, Interrupt service routines
...
{
	// run when new payload received
	packetStatus = PACKET_OK;
}

void nRF905_onRxInvalid(nRF905* device)
{
	// run when corrupted payload recieved
...
{
	Serial.begin(115200);
	Serial.println(F("Base station(Receiver) starting..."));
...
/*****************************************************************************/
	...
	/* 	6,				// CSN SPI Slave select pin, SPI SS. */
	...
	/* 	3, // DR */
	/* 	NRF905_PIN_UNUSED, // Without the AM pin the library the library */
	/* 										 // must poll the status register over SPI. */
	/* 	nRF905_int_dr, */
	...
	//
	// NOTE: In interrupt mode these events will run from the interrupt,
	// so any global variables accessed inside the event function should
	// be declared  'volatile'.  Also  avoid doing  things in  the event
	// functions that take  a long time to complete or  things that rely
	// on interrupts (delay(), millis(), Serial.print())).
	...
	Serial.println(F("Base station(Receiver) started"));
}
...
void loop()
{
	...
	// Atomically copy volatile global variable to local variable since
	// the radio is still in RX mode and another packet could come in at
	// any moment
	noInterrupts();
	...
	else
	{
		...
		// Show received data
		Serial.print(F("Data from or RX client:"));
	...
	/* TODO:  */
	/* // Sleep */
	/* if(packetStatus == PACKET_NONE) // Make sure no new packets were */
	/* 																// received while since the last */
	/* 																// wake */
