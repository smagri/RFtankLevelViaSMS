...
//    nrf905.TX(tx_buf, tx_address);
    ...
    /** Count Sending times */
		...
		// Output to the RX module the string "Hi, ArduinoN \r\n" where N
		// is the number of times we itterate through loop().
    tx_buf[12]++;

		// If N is equal to HEX 3A, which is equivalent to DECIMAL 58 and
		// character ":", reset N to 0.  So with original example code I
		// think we loop forever here.
    if(tx_buf[12] == 0x3A){
      tx_buf[12] = '0';
    }
    delay(50);
